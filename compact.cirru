...
{} (:package |quamolit)
  ...
  :files $ {}
    ...
    |quamolit.app.comp.folding-fan $ %{} :FileEntry
      :defs $ {}
        |comp-folding-fan $ %{} :CodeEntry (:doc |)
          :code $ quote
    ...
    |quamolit.app.comp.icon-increase $ %{} :FileEntry
      :defs $ {}
        |comp-icon-increase $ %{} :CodeEntry (:doc |)
          :code $ quote
    ...
    |quamolit.app.comp.icons-table $ %{} :FileEntry
      :defs $ {}
        |comp-icons-table $ %{} :CodeEntry (:doc |)
          :code $ quote
          ...
          :code $ quote
            defn locate-target (tree coord) (; js/console.log |locating coord tree)
              if (.empty? coord) tree $ let
                ...
                if (&record:matches? Component tree)
                  if
                    = first-pos $ :name tree
            ...
            defn resolve-target (tree event-name coord)
              let
                ...
                if (nil? maybe-target) nil $ let
                    maybe-listener $ if
                      and (record? maybe-target) (&record:matches? maybe-target Shape)
                      get-in maybe-target $ [] :event event-name
                  ; js/console.log |listener maybe-listener maybe-target
    ...
    |quamolit.core $ %{} :FileEntry
      :defs $ {}
        ...
        |handle-event $ %{} :CodeEntry (:doc |)
          :code $ quote
            defn handle-event (coord event-name event dispatch) (; js/console.log "|handle event" coord event-name @*element-tree)
              if-let
    ...
    |quamolit.global $ %{} :FileEntry
      :defs $ {}
        ...
        |*touch-event-areas $ %{} :CodeEntry (:doc |)
          :code $ quote (defatom *touch-event-areas nil)
    ...
    |quamolit.types $ %{} :FileEntry
      :defs $ {}
        |Component $ %{} :CodeEntry (:doc |)
          :code $ quote (defrecord Component :name :on-tick :tree)
        |Shape $ %{} :CodeEntry (:doc |)
          :code $ quote (defrecord Shape :name :style :event :children)
      :ns $ %{} :CodeEntry (:doc |)
