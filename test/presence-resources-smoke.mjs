...
test("100 mount/exit cycles return typed model and host resources to baseline", () => {
  ...
  assert.deepEqual(resources.sync(toJsData(model)), { liveReferences: 0, liveSources: 0, released: 0 });
  ...
  for (let version = 1; version <= 100; version++) {
    ...
    assert.deepEqual(resources.sync(toJsData(model)), { liveReferences: 1, liveSources: 1, released: 0 });
    ...
    const update = resources.sync(toJsData(model));
    ...
    assert.equal(resources.sync(toJsData(model)).released, 0);
  }
...
test("unregistered source rejects model sync without dropping retained source", () => {
  ...
  resources.sync(toJsData(oldModel));
  const newModel = start(instanceDocument(2, true));
  assert.throws(() => resources.sync(toJsData(newModel)), /unavailable/);
  assert.equal(registry.liveCount, 1);
  assert.equal(resources.sync(toJsData(oldModel)).liveSources, 1);
});

test("two live nodes can share one source until their last reference disappears", () => {
  const registry = new InstanceSourceRegistry();
  const resources = new PresenceInstanceResources(registry);
  assert.throws(() => new PresenceInstanceResources(registry), /already has a Presence owner/);
  const source = sourceAt(1);
  registry.register(source, new Float32Array(source.count * 2));
  const base = toJsData(start(instanceDocument(1, true)));
  const instance = base.items.find((item) => item.entry.node.content[0] === "instances");
  const one = { items: [instance] };
  const two = { items: [instance, structuredClone(instance)] };
  assert.deepEqual(resources.sync(two), { liveReferences: 2, liveSources: 1, released: 0 });
  assert.deepEqual(resources.sync(one), { liveReferences: 1, liveSources: 1, released: 0 });
  assert.equal(registry.liveCount, 1);
  assert.deepEqual(resources.clear(), { liveReferences: 0, liveSources: 0, released: 1 });
  assert.equal(registry.liveCount, 0);
});
