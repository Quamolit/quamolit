...
当前默认入口已升级为[保留折线动画](retained-path.md)：静态 Scene、拓扑和局部几何共享，时间只更新变换。下文 `frame-at` 与 `scene-at` 是保留的全量参考，不再是默认播放路径。
...
第一条严格检查 165 个 Calcit 定义、2 项原生测试和 8 项 Node 合同；独立 oracle 用旧版矩阵组合，不复制被测角度累加算法，对 126 条边的端点、宽度、ID 和颜色逐项核对（误差 < 1e-10）。偏移分支负例必须失败。拒绝非法深度/时间，核对 63 次 stroke、状态恢复、整批非法输入无宿主副作用及零宽跳过。另有正式 Scene 身份/序列化、diff 分类、混合层序、能力拒绝与保留计划 1000 帧验证。这是调用计数，不是性能测量。
