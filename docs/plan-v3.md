...
2026-09-26 执行顺序对齐：#50/#33/#104 优先统一标量与几何变换的组件运行入口；随后以 TodoList 联动 #49/#51/#36 的生命周期，再将同一声明接 #38/#40/#52 GPU 合批。其余旧 demo 按共同能力成组恢复，不持续精修摆动树。#113 等次要栅格化差异延期，不阻塞主线；最终验收条件仍保留。#114 待合并内容不是主线已交付能力。

截至 #103 合并：Calcit 0.22.0、js-ffi 0.2.0 已接入；Motion/Scene、直接时间采样与部分 Presence 已有类型和测试。组件采样仍是全量 CPU 参考，保留计划仍由 JS 测试页拼装并引用 target/js/motion，主应用仍是 bootstrap。M0 参考场景性能不能外推为新运行路径性能。

下一笔主线实现应推进 #50：以公共 Calcit 入口接通一个组件声明、Motion 绑定和保留执行计划，同时使用 #33 的 Canvas 参考与 #35 的类型化平台能力。优先落地一个进入/退出、目标打断、稳定 key 重排的组件；可按可运行增量交付，每笔指出推进哪一段用户调用路径。
