...
#118 已合并。后续 #39/#104 三路径测量消费独立 Calcit 模块，记录实际计划采样、批次更新、绘制边界、queue 上传/提交、rAF 与资源计数，见 [消费者帧测量](consumer-performance.md)。当前两个矩形只用于贯通测量链路；下一负载必须扩展公共大规模实例，不能用小负载 p95 代替 10k/画质/资源恢复门禁。
