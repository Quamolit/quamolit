...
后续 GPU 集成分支已经将公共 ComponentPlan 接到有序矩形批次，见 [实现与边界](gpu-component-plan.md)。它是 #38/#40 的基础 CPU 采样→GPU 绘制切片，不等于 #52 标准动画已在 GPU 采样；接下来补静态批次缓存、标准 Motion 参数、独立消费者与同源阶段耗时。GPU 硬件像素成功不能替代资源恢复或性能验收。
