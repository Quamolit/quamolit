...
本分支补充 [Presence 到组件声明的连接](presence-component.md)：矩形/折线局部 alpha、退出禁交互、显式终点结算复用统一计划。它是 TodoList 的前置能力，不是 TodoList 恢复完成；文字、行级 Model、重排过渡、真实输入日志和全屏交互仍待接入。M1/M2 均不因此关闭。
