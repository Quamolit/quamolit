...
[旧 fade 迁移夹具](fade-migration.md)负责展示描述符与 Scene opacity 的直接时间采样；本模块负责逻辑 enter/exit、终点卸载通知与重入。[Presence 组件连接](presence-component.md)现可将顶层矩形/折线转换为统一计划的 alpha 绑定；嵌套组仍未集成，不能把叶节点淡入淡出当作一般组隔离或宿主资源释放。
