...
`PresenceUpdate.released` 是**逻辑释放通知**，携带旧 `SceneEntry`（包括资源 ID/version），供将来的资源表按引用计数或实际宿主所有权释放；它不直接销毁 GPU buffer、Canvas 对象或指针捕获。同一模型结算两次，只有第一次返回通知；若从头重放完整事件日志，会重新得到相同逻辑通知，不能把离线重放结果直接当作新的宿主副作用。`presence-needs-frame?` 在 enter/exit 进行中及终点尚待结算时为 true，调用 `settle-presence` 后变为 false。调用方应在终点安排一次结算帧。
