...
`validate-scene` 在绘制之前拒绝非法数值、颜色、尺寸、资源版本或实例数量、空事件目标、重复节点 ID、同一父级重复 key、缺失/非 group 父节点和不按预序排列的子节点。当前校验是 O(n²) 的正确性参考，不是后续保留执行计划或每帧全量树 diff 的要求。节点的物理 GPU/Canvas 存储槽未定义。[Scene diff 参考实现](scene-diff.md)现已按父路径、key 与节点类型判断复用；跨父或换类型视为卸载与新增。
...
仍待 #32：更完整的资源/图元边界和绑定解析。执行计划与增量调度属于 #50；完整裁剪、透明组和绘制由后续后端 issue 验收。
