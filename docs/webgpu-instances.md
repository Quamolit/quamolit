...
`test/instance-sources.html` 从同一份编译后的 Scene IR 取一个 10k `:instances` 逻辑节点、同一 `(id,version,count)` 位置源与同一颜色/尺寸，分别交给 Canvas 参考路径和 `WebGpuInstanceBatches`。后者只负责从 `InstanceSourceRegistry` 解析不可变 token、缓存 CPU 私有副本、在源版本变化时上传；通用 GPU pipeline、buffer、shader、instanced draw 和诊断读回由已发布的 js-ffi 0.1.43 维护。Quamolit 的 `quamolit.webgpu-batches` 与 `quamolit.instance-ffi` Calcit 命名空间分别引用上游的 GPU 批次及 Float32/Canvas 批次 Calcit API，宿主适配器消费其编译产物，不直接导入这些底层 `.mjs`。Scene IR 不保存 DOM/GPU 句柄。
