...
Quamolit 的 `CanvasInstanceBatches` 是薄适配：从 `InstanceSourceRegistry` 解析不可变 `(id,version,count)` 快照，在某个版本首次绘制时经 `quamolit.instance-ffi/copy-range` 复制一次交错 x/y 数据，之后按 token 缓存该私有副本；每次绘制经 `quamolit.instance-ffi/draw-canvas!` 调用 js-ffi 0.1.43 的 Calcit 批次 API。旧版本释放后即使有缓存也无法继续绘制；新版本获得不同 token 并重新复制。
