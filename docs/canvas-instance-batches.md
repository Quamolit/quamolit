...
通用参数检查、顺序绘制及 `save/restore` 由 js-ffi 维护；Quamolit 只把 Scene 的颜色、尺寸、alpha 和资源引用转换为该通用调用。Canvas 测试页面是正确性参考，不能代替 #33 的完整 Scene 后端，也不能代替 #40 的 WebGPU 主路径。WebGPU 能力探测、buffer 上传、设备丢失与回退仍属于 #35/#51/#40 后续切片。
