...
这是以 O(n²) 匹配为主的**正确性参考**；前置深树校验最坏可达 O(n³)。它不是每帧执行计划，也不能替代 #50 的保留式增量调度。兄弟序号变化会保守标记 `order`，即使只是前面插入一个节点；后端可利用相对顺序进一步缩小实际重排。透明内容的原始预序绘制次序必须保留。当前 `SceneChange` 只报告逻辑差异，不持有或销毁宿主资源；绑定求值另有 [CPU 参考实现](scene-binding.md)，而 typed array 脏范围、实例源上传、局部 hit-test、Canvas/WebGPU 增量执行都尚未实现。
