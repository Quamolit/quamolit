...
`yarn test:motion` 对实验性标量、二维向量、关键帧与颜色 Motion 做严格类型检查、原生测试、JS 编译与数值测试；`yarn test:motion-browser` 再验证 Chromium 中间帧、循环端点、颜色像素和时间倒退/重复。两者目前只覆盖 [初始切片](motion-scalar.md)，不能代表 #48 全部 Motion IR 或 #31 生命周期已完成。

## 待实现的统一命令

| 命令 | 负责工作项 | 完成条件 |
| --- | --- | --- |
| 完整 Motion/过渡测试 | #48、#31 | 在标量/二维向量/关键帧/颜色切片之上补齐有界组合与过渡生命周期，覆盖模拟/打断边界 |
