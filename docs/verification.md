...
当前 `yarn compile:visual` 只编译，不自动运行像素检查。M0 完成前，手工浏览器证据必须注明时间/画面/结果。PR #45 是否已合并应另行核对；本规则不预设其代码在 main。

## 待实现的统一命令

| 命令 | 负责工作项 | 完成条件 |
| --- | --- | --- |
| `yarn test:visual` | M0 最小视觉 CI；#37 扩展 | 固定浏览器执行断言，失败上传 actual/expected/diff 和 manifest |
