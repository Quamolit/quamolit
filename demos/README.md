...
`test:demo-nav` 首先核对页面清单完整性、重复路径、说明文件、编译入口与返回链接，再编译/打包；浏览器只能访问 `dist-demos`，在 `/preview/` 子路径下依次从导航打开 26 个入口、检查状态/网络/运行错误并返回。额外验证搜索、分类、刷新与移动端布局，共 27 个浏览器用例。GPU 在这个门禁中强制不可用，仅验证初始化及 Canvas 回退，不替代真实 GPU 验收。根占位/基准宿主页只有加载检查，其余有相应初始化状态断言。
