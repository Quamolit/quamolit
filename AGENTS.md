...
- 探索阶段优先贯通整体功能和框架，以可演示的完整用户路径交付，避免每个小前置都单独停顿。编译失败、主路径不可用和核心语义错误及时修复；次要画质/边界/局部优化记录到 GitHub issue，写明影响、复现和后续验收，不阻塞主线。保留核心回归与 Actions 门禁；容差变更必须有数据、差异图和 issue，不得静默放宽。

- 旧示例按 `docs/demo-restoration.md` 全部恢复，不用技术夹具替代。未来动画默认全屏 Canvas + 可收起 DOM 浮层；固定尺寸只用于明确标注的诊断夹具。导航 planned 与已实现 entries 分开，恢复时保持稳定 ID；艺术作品不得以占位卡冒充完成。

- 新增或移动 demo 时必须同步 `demos/catalog.json` 的分类、能力边界、文档与编译入口，并提供返回导航的链接。运行 `yarn test:demo-nav` 验证完整静态产物中的导航往返；不能只在 PR 中贴临时地址。独立消费者搬离仓库时导航可退回说明链接，不新增框架内部 JS 依赖。
